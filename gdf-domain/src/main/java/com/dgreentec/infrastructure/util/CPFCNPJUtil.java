...
public class CPFCNPJUtil {
	...
	private static final String VALUE_IS_NOT_A_VALID_CPF_OR_CNPJ = "value is not a valid CPF or CNPJ";
	...
	public static String formatCPForCNPJ(String value) {
		...
		return formatCPForCNPJ(Long.parseLong(value.replaceAll("[^0-9]+", "")), true);
	}
	 ...
	 * @return CPF ou CNPJ formatado
	 */
	public static String formatCPForCNPJ(String value, boolean check) {
		if (value == null || value.isEmpty()) {
			throw new IllegalArgumentException(VALUE_CANNOT_BE_NULL_OR_EMPTY);
		}
		return formatCPForCNPJ(Long.parseLong(value.replaceAll("[^0-9]+", "")), check);
	}
	 ...
	 * @return CPF ou CNPJ formatado
	 */
	public static String formatCPForCNPJ(Long value) {
		return formatCPForCNPJ(value, true);
	}
	 ...
	 * @return CPF ou CNPJ formatado
	 */
	public static String formatCPForCNPJ(Long value, boolean check) {
		if (value == null) {
		...
		if (check && !isCPForCNPJ(value)) {
			throw new IllegalArgumentException(VALUE_IS_NOT_A_VALID_CPF_OR_CNPJ);
		}
	...
	public static boolean isCPForCNPJ(String value) {
		...
		return isCPForCNPJ(Long.parseLong(value.replaceAll("[^0-9]+", "")));
	}
	 ...
	 * @return [boolean] true caso seja um valor válido, false caso contrário
	 */
	public static boolean isCPForCNPJ(Long value) {
	...
	/**
	 * Calcula digito verificador para CPF or CNPJ
	 *
	 * @param stringBase
